...
$student = mysqli_fetch_array($query_run);
     $lab = $student['curlab'];
     $imagePath = 'images/profile/jegii.png'; // Replace with your image path
...
<body>
             ...
             <div class="page-breadcrumb">
                ...
                <input type="text" id="session" placeholder="session...">
                    <table id="myTable1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <?php
    $query = "SELECT * FROM attendance_records where lab='$lab'";
    ...
    ?>
					...
					<img id="studentImage" src="data:image/jpeg;base64,<?php echo $imageData; ?>" alt="Student Image">
...
<script>
        // JavaScript code to set the fetched image data in the img element
        const imageData = '<?php echo $imageData; ?>';
        const studentImage = document.getElementById('studentImage');
        studentImage.src = 'data:image/jpeg;base64,' + imageData;
</script>
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        generatePdfButton.addEventListener('click', function () {
            ...
            const imageUrlTopRight = 'images/mkcelogo.png'; // Relative path to your second image
            ...
            imageTopLeft.onload = function () {
                ...
                imageTopRight.onload = function () {
                    ...
                    // Fetch the image from the database and add it to the PDF, resizing it to passport size
                    const imageFromDatabase = new Image();
                    imageFromDatabase.src = '<?php echo $imagePath; ?>';
                    imageFromDatabase.onload = function () {
                        const passportWidth = 140; // Set the desired passport photo width
                        const passportHeight = 140; // Set the desired passport photo height

                        const canvasDatabase = document.createElement('canvas');
                        canvasDatabase.width = passportWidth;
                        canvasDatabase.height = passportHeight;
                        const ctxDatabase = canvasDatabase.getContext('2d');

                        // Draw the fetched image on the canvas with the new dimensions
                        ctxDatabase.drawImage(imageFromDatabase, 0, 0, passportWidth, passportHeight);

                        // Convert the resized image to a data URL
                        const dataUrlDatabase = canvasDatabase.toDataURL('image/png');

                        // Generate PDF using pdfmake with all three images and organized text
                        const pdfDefinition = {
                            pageSize: 'A4',
                            pageMargins: [0, 0, 0, 0], // Set margins to 0 to place the images at the corners
                            content: [
                                {
                                    image: dataUrlTopLeft, // Specify the first image data URL
                                    width: 100, // Adjust the width as needed
                                    absolutePosition: { x: 50, y: 20 } // Adjust the position (top left corner)
                                },
                                {
                                    image: dataUrlTopRight, // Specify the second image data URL
                                    width: 100, // Adjust the width as needed
                                    absolutePosition: { x: 400, y: 20 } // Adjust the position (top right corner)
                                },
                                {
                                    text: 'Dr. B.S.Murugan, M.Tech, Ph.D Principal', // Add the text below the top-left image
                                    absolutePosition: { x: 50, y: 140 }, // Adjust the position
                                    fontSize: 12, // Adjust the font size as needed
                                    bold: true, // Make the text bold
                                    alignment: 'left' // Align the text to the left
                                },
                                {
                                    text: 'Bonafide Certificate', // Add the text below the top-right image
                                    fontSize: 12, // Adjust the font size as needed
                                    bold: true, // Make the text bold
                                    alignment: 'center', // Align the text to the center
                                    margin: [0, 100] // Add some space below the heading
                                },
                                {
                                    text: 'This is to certify that JEGAN VC (Reg No : 927621BCS046, Aadhar No : 200547402002) S/O Mr. CHANDRASEKARAN R is a bonafide student of our College, Studying in III Year B.E COMPUTER SCIENCE AND ENGINEERING during academic year 2023 - 2024',
                                    fontSize: 12,
                                    alignment: 'center', // Align the text to the center
                                    margin: [50, 20] // Add some space below the text
                                    lineHeight: 1.5 // Set the line spacing to 1.5
                                },
                                {
                                    image: dataUrlDatabase, // Specify the fetched and resized image data URL
                                    width: passportWidth, // Set the width to the desired passport size
                                    height: passportHeight, // Set the height to the desired passport size
                                    absolutePosition: { x: 400, y: 400 } // Adjust the position (bottom)
                                }
                            ]
                        };

                        pdfMake.createPdf(pdfDefinition).download('image_with_text.pdf');
                    };
                };
            };
        });
